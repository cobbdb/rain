...
ELabel.prototype.redraw=function(force){var p=this.map_.fromLatLngToDivPixel(this.point);var h=parseInt(this.div_.clientHeight);this.div_.style.left=(p.x+this.pixelOffset.width)+"px";this.div_.style.top=(p.y+this.pixelOffset.height-h)+"px";}
...
function getRainfall(mo_index){var str_val=stringToInt($("#io_rain_inputs_"+mo_index+"_box").val());str_val/=unit_mm[current_unit];return str_val;}
function getUsage(mo_index){var str_val=stringToInt($("#input_usage_"+mo_index+"_box").val());str_val/=unit_L[current_unit];return str_val;}
function getArea(){var val_area=stringToInt($("#io_input_area").val());val_area/=unit_m2[current_unit];return val_area;}
function getEfficiency(){var eff=$("#input_material_box").val();eff=parseFloat(eff);return eff;}
function getDistance(){var i_dist=stringToInt($("#input_storage_box").val());i_dist/=unit_m[current_unit];return i_dist;}
function getTankInput(){var tank=stringToInt($("#input_tank_box").val());tank/=unit_L[current_unit];return tank;}
function getTankSize(){var tank_size=calc_storage_all(false);if(tank_size==-99){var max_level=storage_data[0];for(var i=1;i<12;i+=1){max_level=Math.max(max_level,storage_data[i]);}
if(max_level<storage_max){max_level=Math.max(max_level,storage_min);tank_size=max_level;}else{tank_size="4000000";}}else if(tank_size==-88){tank_size="10";}
return tank_size;}
function setAnnualHarvest(){var rain=p_year/1000;var area=getArea();var eff=getEfficiency();var harvest=rain*area*eff*1000;harvest*=unit_L[current_unit];harvest=Math.round(harvest);$("#out_harvest").html(harvest+" "+unit_name_L[current_unit]);}
function setTankWeight(){var tank=getTankInput();tank*=0.001;var weight=tank*1000;weight*=unit_kg[current_unit];weight=roundToFixed(weight,0);$("#out_weight").html(weight+" "+unit_name_kg[current_unit]);}
function setFirstFlush(){var roof=getArea();roof*=factor_m22ft2;var gal_low=roof*1/100;var gal_high=roof*2/100;gal_low/=factor_L2gal;gal_high/=factor_L2gal;gal_low*=unit_L[current_unit];gal_high*=unit_L[current_unit];gal_low=roundToFixed(gal_low,1);gal_high=roundToFixed(gal_high,1);$("#out_first_flush").html(gal_low+" - "+gal_high+" "+unit_name_L[current_unit]);}
function getHeadLossValues(){var heightMin=0.0833333333;var heightMax=15;var flow=calcFlow();var length=getDistance();length*=factor_m2ft;var exp=1/4.87;var coef=0.00113*length*Math.pow(flow,1.85);var d_max=Math.pow(coef/heightMin,exp);var d_min=Math.pow(coef/heightMax,exp);var head_loss=[];var diam,height,index=0;for(var diam=d_min;diam<d_max;diam+=0.05){height=coef/Math.pow(diam,4.87);height/=factor_m2ft;height*=unit_m[current_unit];height=roundToFixed(height,2);diam=roundToFixed(diam,2);head_loss[index]=[diam,height];index+=1;}
return head_loss;}
function calcFlow(){var area=getArea();var max_rainfall=50;max_rainfall/=1000;max_rainfall/=60;var flow=area*max_rainfall;flow*=264.172052;return flow;}
function calc_capacity_mo(mo_index){var i_area=getArea();var i_rain=getRainfall(mo_index);var i_eff=getEfficiency();i_rain/=1000;return(i_area*i_rain*i_eff)*1000;}
function calc_storage_mo(mo_index,prev_mo,tank_size){var i_use=getUsage(mo_index);var i_cap=calc_capacity_mo(mo_index);var i_ideal=i_cap+prev_mo-i_use;return Math.min(i_ideal,tank_size);}
function calc_supply(mo_index,full_tank){var stored;var usage=getUsage(mo_index);var capacity=calc_capacity_mo(mo_index);if(mo_index==0){stored=(full_tank)?current_tank_size:0;}else{stored=storage_data[mo_index-1];}
var supply=stored+capacity;supply=Math.min(supply,usage);return supply;}
function sizeIsGood(tank_size,full_tank){var this_mo;var prev_mo;var size_is_good=true;prev_mo=(full_tank)?tank_size:0;for(var i=0;i<12;i+=1){this_mo=calc_storage_mo(i,prev_mo,tank_size);if(this_mo<=min_tank_val){size_is_good=false;break;}
prev_mo=this_mo;}
return size_is_good;}
var storage_max=4000000;var storage_min=10;var min_tank_val=5;function calc_storage_all(full_tank){var tank_min=0,tank_max=storage_max;var tank_size=tank_min;var good_size;var i,exit=false;var err_code=0;do{tank_size=(tank_min+tank_max)/2;tank_size=Math.round(tank_size);good_size=sizeIsGood(tank_size,full_tank);if(tank_min==tank_max){err_code=1;exit=true;}else if(good_size&&tank_size<storage_min){err_code=-1;exit=true;}else if(good_size&&tank_size==tank_max){err_code=0;exit=true;}
if(good_size){tank_max=tank_size;}else{tank_min=tank_size;}}while(!exit);if(err_code>0){tank_size=-99;}else if(err_code<0){tank_size=-88;}
loadStorageValues(tank_size,full_tank);return tank_size;}
var storage_data=[];function loadStorageValues(tank_size,full_tank){var this_mo;if(tank_size==-99){tank_size=storage_max;}else if(tank_size==-88){tank_size=storage_min;}
var prev_mo=(full_tank)?tank_size:0;for(var i=0;i<12;i+=1){this_mo=calc_storage_mo(i,prev_mo,tank_size);this_mo=Math.max(this_mo,0);storage_data[i]=Math.round(this_mo);prev_mo=this_mo;}}
var map=null;var geocoder=null;var roofareanew,percentflushes;var poly;var count=0;var points=[];var markers=[];var icon_url="./";var tooltip;var report=document.getElementById("out_area");var mapExtension,identifyTask,layers,overlays;var mapLocked=false;var json;var s_dist=0;function addIcon(icon){icon.shadow=icon_url+"mm_20_shadow.png";icon.iconSize=new GSize(12,20);icon.shadowSize=new GSize(22,20);icon.iconAnchor=new GPoint(6,20);icon.infoWindowAnchor=new GPoint(5,1);}
...
var currtype=map.getCurrentMapType().getProjection();var point=currtype.fromLatLngToPixel(map.fromDivPixelToLatLng(new GPoint(0,0),true),map.getZoom());var offset=currtype.fromLatLngToPixel(marker.getLatLng(),map.getZoom());var anchor=marker.getIcon().iconAnchor;var width=marker.getIcon().iconSize.width+6;var height=10;var pos=new GControlPosition(G_ANCHOR_TOP_LEFT,new GSize(offset.x-point.x-anchor.x+width,offset.y-point.y-anchor.y-height));pos.apply(tooltip);}
function CustomGetTileUrlRain(a,b){return"http://www.laudontech.com//save-the-rain//Z"+b+"/"+a.y+"_"+a.x+".png";return G_PHYSICAL_MAP.getTileLayers()[0].getTileUrl(a,b);}
function initialize(){var container=document.getElementById("mapcontent");map=new GMap2(container,{draggableCursor:"crosshair",draggingCursor:"move"});mapExtension=new esri.arcgis.gmaps.MapExtension(map);tooltip=document.createElement("div");tooltip.className="tooltip";map.getPane(G_MAP_MARKER_PANE).appendChild(tooltip);map.setUIToDefault();map.setMapType(G_HYBRID_MAP);map.setCenter(new GLatLng(40.872738,-124.077417),18);var copyrightCollection=new GCopyrightCollection('Map Data:')
var copyright=new GCopyright(1,new GLatLngBounds(new GLatLng(-90,-180),new GLatLng(90,180)),0,"Rainfall");copyrightCollection.addCopyright(copyright);var tilelayersRain=[];tilelayersRain[0]=new GTileLayer(copyrightCollection,4,11);tilelayersRain[0].getTileUrl=function(){return"http://www.laudontech.com//zsevanston//blank2.png";};tilelayersRain[0].getOpacity=function(){return 1.0;};tilelayersRain[0].isPng=function(){return true;};tilelayersRain[1]=G_NORMAL_MAP.getTileLayers()[0];tilelayersRain[2]=new GTileLayer(copyrightCollection,3,11);tilelayersRain[2].getTileUrl=CustomGetTileUrlRain
tilelayersRain[2].getOpacity=function(){return 0.7;};tilelayersRain[2].isPng=function(){return true;};var GMapTypeOptions={};GMapTypeOptions.minResolution=1;GMapTypeOptions.maxResolution=11;GMapTypeOptions.errorMessage="No map data available";var custommapRain=new GMapType(tilelayersRain,new GMercatorProjection(24),"Rainfall",GMapTypeOptions);custommapRain.getTextColor=function(){return"#ffffff";};map.addMapType(custommapRain);geocoder=new GClientGeocoder();map.disableDoubleClickZoom();GEvent.addListener(map,"click",leftClick);GEvent.addListener(map,"maptypechanged",function(){var newMapType=map.getCurrentMapType().getName(true);if(newMapType=="Rainfall"){var zoomlevel=map.getZoom();if(zoomlevel>=10){map.setZoom(9);}}});}
function leftClick(overlay,point){if(point&&!mapLocked){count++;var icon=new GIcon();icon.image=icon_url+"mm_20_red.png";addIcon(icon);var marker=new GMarker(point,{icon:icon,draggable:true,bouncy:false,dragCrossMove:true});map.addOverlay(marker);marker.content=count;markers.push(marker);marker.tooltip="Point "+count;if(markers.length==3){enableDoneButton();$("#io_reset").show();}
if(markers.length>=3){$("#io_input_area").attr("disabled",false);}
GEvent.addListener(marker,"mouseover",function(){});GEvent.addListener(marker,"mouseout",function(){tooltip.style.display="none";});GEvent.addListener(marker,"drag",function(){tooltip.style.display="none";drawOverlay();});GEvent.addListener(marker,"click",function(){tooltip.style.display="none";for(var n=0;n<markers.length;n+=1){if(markers[n]==marker){map.removeOverlay(markers[n]);break;}}
markers.splice(n,1);if(markers.length==0){count=0;}else{count=markers[markers.length-1].content;drawOverlay();}});drawOverlay();}}
function drawOverlay(){if(poly){map.removeOverlay(poly);}
points.length=0;for(var i=0;i<markers.length;i+=1){points.push(markers[i].getLatLng());}
points.push(markers[0].getLatLng());poly=new GPolygon(points,"#FF0000",2,0.9,"#FF0000",0.2);roofarea=poly.getArea();var unit="m&sup2;";roofareanew=roofarea.toFixed(1);map.addOverlay(poly);$("#io_input_area").val(Math.round(roofarea.toFixed(1)*unit_m2[current_unit]));newArea();}
function clearMap(){map.clearOverlays();$("#out_harvest").html("");points.length=0;markers.length=0;count=0;$("#out_area").html("");$("#out_rain").html("");clearDoneButton();clearResetArrow();$("#io_reset").hide();json=null;clearAllValues();mapLocked=false;}
function removeAll(){map.clearOverlays();var boxes=document.getElementsByName("box");for(var i=0;i<boxes.length;i+=1){boxes[i].checked=false;}}
function callWeather(){$("#b_addr").attr("disabled",true);$("#addressbox").attr("disabled",true);$("#io_reset").hide();$("#out_rain").html("..pending..");$("#out_harvest").html("..pending..");clearDoneButton();$("#b_reset").attr("disabled",true);clearResetArrow();mapLocked=true;$("#io_input_area").attr("disabled",true);var map_center=map.getCenter();$.ajax({url:"./rain.php",data:("lat="+map_center.lat()+"&"+"lng="+map_center.lng()),type:"GET",cache:false,dataType:"json",error:function(xhr){alert(xhr.status+" : "+xhr.statusText);$("#b_reset").attr("disabled",false);enableResetArrow();$("#b_done[rel]").data("overlay").close();},success:function(json_response){json=json_response;for(var i=0;i<markers.length;i+=1){markers[i].hide();}
polycenter=poly.getBounds().getCenter();map.setCenter(polycenter);var val_convert=0;p_year=stringToInt(json["p_year"]);s_dist=stringToInt(json["s_dist"]);s_dist=roundToFixed(s_dist,2);val_convert=roundToFixed(p_year*unit_mm[current_unit],1);$("#out_rain").html(val_convert+" "+unit_name_mm[current_unit]);setAnnualHarvest();showDirections(polycenter,json["s_lat"],json["s_lng"]);$("#rain_graph_link").show();$("#b_addr").attr("disabled",false);$("#addressbox").attr("disabled",false);$("#b_reset").attr("disabled",false);$("#io_reset").show();$("#b_done[rel]").data("overlay").close();setPrecipitationBoxes(true);loadPrecipitationValues();$("#io_input_area").attr("disabled",false);updateTankSize();}});}
function loadPrecipitationValues(){var rain_val;for(var i=0;i<12;i+=1){rain_val=stringToInt(json["p_mo_"+i])
rain_val*=unit_mm[current_unit];rain_val=roundToFixed(rain_val,1);$("#io_rain_inputs_"+i+"_box").val(rain_val);}}
function showDirections(gl_src,dest_lat,dest_lng){var gl_dest=new GLatLng(dest_lat,dest_lng);var icon=new GIcon();icon.image=icon_url+"mm_20_red.png";addIcon(icon);var gm_station=new GMarker(gl_dest,{icon:icon});var gm_roof=new GMarker(gl_src,{icon:icon});map.addOverlay(gm_station);map.addOverlay(gm_roof);var gp_line=new GPolyline([gl_dest,gl_src]);map.addOverlay(gp_line);map.setCenter(gp_line.getBounds().getCenter());map.setZoom(map.getBoundsZoomLevel(gp_line.getBounds()));var el_html='<div style="color:black;background-color:#ccccff;border:2px solid black">Station<\/div>';var el_station=new ELabel(gl_dest,el_html,null,new GSize(-24,28),75);el_html='<div style="color:black;background-color:#ccccff;border:2px solid black">&nbsp;You&nbsp;<\/div>';var el_source=new ELabel(gl_src,el_html,null,new GSize(-18,28),75);map.addOverlay(el_station);map.addOverlay(el_source);}
function showAddress(address){var address=$("#addressbox").val();clearMap();if(geocoder){geocoder.getLatLng(address,function(point){if(!point){alert(address+" not found");}else{map.setMapType(G_HYBRID_MAP);map.setCenter(point,17);map.clearOverlays();}});}}
function mapInit(){initialize();json={s_lat:"40.872738",s_lng:"-124.077417",s_dist:"11.23",p_year:"991",p_mo_0:"171",p_mo_1:"145",p_mo_2:"133",p_mo_3:"80",p_mo_4:"45",p_mo_5:"18",p_mo_6:"3",p_mo_7:"6",p_mo_8:"21",p_mo_9:"64",p_mo_10:"135",p_mo_11:"170"};s_dist=11.23;}
var RainController=function($scope){$scope.approUrl='http://www.appropedia.org/Rainwater_Collection_Calculator';$scope.dismiss=function($event){var target=$($event.target).attr('data-dismiss');$(target).slideUp({queue:false}).fadeOut({queue:false});};}
var u_metric=0;var u_us=1;var current_unit=u_us;var factor_L2gal=0.264172052;var factor_mm2in=0.0393700787;var factor_km2mi=0.621371192;var factor_m22ft2=10.7639104;var factor_m2ft=3.2808399;var factor_kg2lb=2.20462262;var unit_L=[1,factor_L2gal];var unit_mm=[1,factor_mm2in];var unit_km=[1,factor_km2mi];var unit_m=[1,factor_m2ft];var unit_m2=[1,factor_m22ft2];var unit_kg=[1,factor_kg2lb];var unit_name_L=["L","gal"];var unit_name_mm=["mm","in"];var unit_name_km=["km","mi"];var unit_name_m=["m","ft"];var unit_name_m2=["m&sup2;","ft&sup2;"];var unit_name_kg=["kg","lb"];function toggleGlobalUnit(){var new_unit=(current_unit==u_us)?u_metric:u_us;setGlobalUnit(new_unit);if($("#graph_area").is(":visible")){changeGraphic(currentGraphic);}}
function setGlobalUnit(new_unit){var old_unit=current_unit;current_unit=new_unit;$("#unit_setting").html((current_unit)?"US":"metric");$(".unit_mm").html(unit_name_mm[current_unit]);$(".unit_m").html(unit_name_m[current_unit]);$(".unit_km").html(unit_name_km[current_unit]);$(".unit_L").html(unit_name_L[current_unit]);$(".unit_m2").html(unit_name_m2[current_unit]);convertArea(old_unit);convertPrecipitation(old_unit);convertDistance(old_unit);convertUsage(old_unit);}
function convertArea(old_unit){var val_area=stringToInt($("#io_input_area").val());val_area/=unit_m2[old_unit];val_area*=unit_m2[current_unit];val_area=roundToFixed(val_area,1);$("#io_input_area").val(val_area);$("#out_area").html(val_area+" "+unit_name_m2[current_unit]);setFirstFlush();}
function convertDistance(old_unit){var val_dist=stringToInt($("#input_storage_box").val());val_dist/=unit_m[old_unit];val_dist*=unit_m[current_unit];val_dist=roundToFixed(val_dist,1);$("#input_storage_box").val(val_dist);$("#out_flush_dist").html(val_dist+" "+unit_name_m[current_unit]);}
function convertPrecipitation(old_unit){var val_rain;for(var i=0;i<12;i+=1){val_rain=stringToInt($("#io_rain_inputs_"+i+"_box").val());val_rain/=unit_mm[old_unit];val_rain*=unit_mm[current_unit];val_rain=roundToFixed(val_rain,1);$("#io_rain_inputs_"+i+"_box").val(val_rain);}
newPrecipitation();}
function convertUsage(old_unit){var val_use;for(var i=0;i<12;i+=1){val_use=stringToInt($("#input_usage_"+i+"_box").val());val_use/=unit_L[old_unit];val_use*=unit_L[current_unit];val_use=roundToFixed(val_use,0);$("#input_usage_"+i+"_box").val(val_use);}
newUsage();}